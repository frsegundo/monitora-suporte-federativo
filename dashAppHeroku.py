...
def retonarDf(EstadoAlvo,nomeMeses,ICMS,compilado):
  ...
  if dadosEstado[5][-1] >= 0 :
    dataF = pd.DataFrame({label: ['Não perdeu arrecadação.']}, index=[EstadoAlvo])  
  else:
    dataF = pd.DataFrame({label: [-1*sum(dadosEstado[7])/dadosEstado[5][-1]]}, index=[EstadoAlvo])
  
  listadeDF.append(dataF)

  #quarto item
  if dadosEstado[5][-1] >= 0 :
    testeSuficiencia = 'Não perdeu arrecadação.' 
  else:
    testeSuficiencia=-1*sum(dadosEstado[7])/dadosEstado[5][-1]
  # montando o dataframe que vai ser mostrado ao lado do gráfico
  dataTab=[]
  ...
  dataTab.append(['Suficiência do Suporte',testeSuficiencia])
  listadeDF.append(pd.DataFrame(dataTab,columns=nomeColunas))

  # quinto item
  dataF = pd.DataFrame({'Recursos MP938': [dadosEstado[7][0]],
                   'Recursos LC 173': [dadosEstado[7][1]],
                   'Suspensão de Dívida LC173 (2)': [dadosEstado[7][2]],
                   'Total do Suporte':[sum(dadosEstado[7])],
                   'ICMS Perdido':[dadosEstado[5][-1]]}, index=[EstadoAlvo])
  listadeDF.append(dataF)
...
def update_output(value):
    ...
    for valor in eixos[2]:
      if valor>=0:
        coresBarra.append('rgb(26, 118, 255)')
      else:
